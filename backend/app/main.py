...
DISPLAY = {"age_years":"Age", "gender":"Sex", "height":"Height", "weight":"Weight", "ap_hi":"Systolic BP", "ap_lo":"Diastolic BP", "cholesterol":"Cholesterol", "gluc":"Glucose", "smoke":"Smoking", "alco":"Alcohol", "active":"Physical activity", "bmi":"BMI"}
...
def explain(frame: pd.DataFrame):
    ...
    try:
        ...
        logger.warning("SHAP unavailable (%s); using model feature importances only", exc)
        # Safe operational fallback if a model runtime lacks compatible SHAP binaries.
    ...
    for feature, value, contribution in zip(FEATURES, frame.iloc[0], values):
        ...
        items.append({"feature": feature, "label": DISPLAY[feature], "value": round(float(value), 1), "contribution": round(c, 4), "direction": "risk_increasing" if c >= 0 else "protective", "group": "modifiable" if feature in MODIFIABLE else "non_modifiable"})
    items.sort(key=lambda x: abs(x["contribution"]), reverse=True)
    return {"method": method, "modifiable": [x for x in items if x["group"] == "modifiable"][:5], "non_modifiable": [x for x in items if x["group"] == "non_modifiable"][:3]}

def assessment(patient: Patient | dict):
    frame, d = as_frame(patient)
    prob = float(state["model"].predict_proba(frame)[0, 1])
    return {"patient": d, "prediction": {"risk_probability": round(prob, 4), "risk_score": round(prob * 100, 1), "risk_category": risk_category(prob), "model_used": state["meta"]["best_model"]}, "explanation": explain(frame), "indicators": {"bmi": d["bmi"], "blood_pressure": f'{int(d["ap_hi"])} / {int(d["ap_lo"])} mmHg', "cholesterol": ["Normal", "Above normal", "High"][int(d["cholesterol"])-1], "glucose": ["Normal", "Above normal", "High"][int(d["gluc"])-1], "smoking": "Yes" if d["smoke"] else "No"}}
...
app = FastAPI(title="CVD Clinical Decision Support MVP", version="0.1.0", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

@app.get("/api/health")
def health(): return {"status":"ok", "model":state["meta"]["best_model"], "training_rows":state["meta"]["training_rows"]}
@app.get("/api/models")
def models(): return state["meta"]
@app.get("/api/patients")
def get_patients(): return state["patients"]
@app.post("/api/assessment")
def get_assessment(patient: Patient): return assessment(patient)
@app.post("/api/predict")
def predict(patient: Patient): return assessment(patient)["prediction"]
@app.post("/api/explain")
def get_explanation(patient: Patient): return assessment(patient)["explanation"]
@app.post("/api/what-if")
def what_if(payload: WhatIf):
    before = assessment(payload.baseline)
    valid = MODIFIABLE - {"bmi"}
    illegal = set(payload.changes) - valid
    if illegal: raise HTTPException(422, f"Only modifiable fields may change: {', '.join(sorted(illegal))}")
    changed = payload.baseline.model_dump()
    changed.update(payload.changes)
    after = assessment(changed)
    return {"current": before["prediction"], "simulated": after["prediction"], "difference_points": round(after["prediction"]["risk_score"] - before["prediction"]["risk_score"], 1), "assessment": after}
@app.get("/api/who/benchmark")
def who_benchmark():
    return {"status":"unavailable", "message":"WHO GHO live benchmark is not configured for this MVP deployment. The dashboard intentionally does not display substitute values."}
@app.websocket("/ws/patient/{patient_id}")
async def telemetry(websocket: WebSocket, patient_id: str):
    await websocket.accept()
    try:
        while True:
            await websocket.send_json({"patient_id":patient_id, "kind":"simulated_telemetry", "systolic_bp":random.randint(118, 150), "diastolic_bp":random.randint(75, 95), "heart_rate":random.randint(62, 92)})
